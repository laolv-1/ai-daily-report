...
try:
    with urllib.request.urlopen('http://127.0.0.1:9222/json/version', timeout=3) as r:
        result['version'] = json.loads(r.read().decode('utf-8','ignore'))
        result['cdp_ready'] = True
except Exception as e:
    result['error'] = 'version_probe_failed:' + repr(e)
...
if result['cdp_ready']:
    try:
        ...
        result['error'] = 'cdp_connect_or_check_failed:' + repr(e)
...
sftp=cli.open_sftp()
remote=r'C:\Users\Administrator\cdp_verify_login.py'
with sftp.open(remote,'w') as f:
    f.write(textwrap.dedent(remote_script).strip()+"\r\n")
stdin, stdout, stderr = cli.exec_command(r'cmd /c python C:\Users\Administrator\cdp_verify_login.py', timeout=300)
